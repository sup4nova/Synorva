...
# Load .env before any os.getenv() call further down
...
# --- Dataset helpers (called from __main__ block) ---
...
async def analyze_image(file: UploadFile = File(...)):
    ...
    Used by the frontend to display the detected emotion without generating audio.
    """
    if not file.content_type or not file.content_type.startswith("image/"):
...
async def build_track(file: UploadFile = File(...)):
    ...
    pred = predict_valaro_from_bgr(img, file.filename)   # {valence, arousal, features_used}
    info = render_track(target_val=pred["valence"], target_aro=pred["arousal"])

    # Build a /static/... URL the browser can fetch directly
    ...
    return {
        ...
        "mix_url": mix_url,      # e.g. /static/renders/mix.wav
        "picks":   info["picks"],
    }
...
if __name__ == "__main__":
    ...
    if args.mode in ("audio", "both"):
        ...
        build_audio_features_csv()   # scan raw_audio/, extract features, label, save CSVs

    if args.mode in ("image", "both"):
        print("\nIMAGE step 1/2: features...")
        build_image_features_csv()   # scan raw_picture/, extract visual features
        print("IMAGE step 2/2: labeling...")
        label_image_features_csv()   # add valence/arousal/emotion columns
